...
// app.js (KODE PERBAIKAN UNTUK MASALAH RESET HARIAN)
 ...
 * Fungsi ini harus dipanggil di window.onload dan secara berkala.
 */
...
async function checkCardSupabase(rfidId) {
    // 0. HAPUS: setupInitialState() TIDAK DIPERLUKAN di sini lagi.
    // Logika reset sudah ditangani di window.onload dan setInterval.
    ...
    try {
        ...
        if (userData) {
            ...
            // Catatan: Karena `getTodayDateString` sudah disesuaikan untuk reset jam 1 pagi, 
            // query Supabase dengan tanggal tersebut sudah tepat.
...
function setupDailyReset() {
    // Hitung berapa lama lagi sampai 01:00 hari berikutnya
    const now = new Date();
    ...
    setTimeout(() => {
        setupInitialState(); // Panggil fungsi reset
        // Set interval untuk hari-hari berikutnya, dipanggil setiap 24 jam sekali
...
window.onload = () => {
    // 1. Setup state awal (memuat dari Local Storage atau mereset jika sudah lewat 01:00)
    setupInitialState(); 
    
    // 2. Setup listener untuk reset harian (agar counter direset TEPAT pukul 01:00 meskipun tidak ada aktivitas)
    setupDailyReset();
    
    // 3. Setup HID listener
    setupHIDListener();
};
