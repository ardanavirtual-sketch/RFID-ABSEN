...
function showProcessingStatus() {
    // ... (Fungsi showProcessingStatus tetap sama) ...
...
// Fungsi BARU untuk Memutar Audio dengan Logging
const playSound = (audioElement, status) => {
    // Coba memuat ulang audio jika mungkin ada masalah cache
    audioElement.load(); 
    audioElement.currentTime = 0; // Reset ke awal
    audioElement.play().then(() => {
        console.log(`[AUDIO SUKSES]: Berhasil mencoba memutar suara untuk status: ${status}`);
    }).catch(e => {
        console.error(`[AUDIO GAGAL]: Gagal memutar audio ${status}. Cek Autoplay Policy, nama file/path, atau format audio (.mpeg) tidak didukung:`, e);
    });
};
...
function updateUI({ success, message, rfidId, nama, status_log }) {
    ...
    if (success) {
        playSound(successSound, 'SUKSES');
        // ... (Logika UI Sukses tetap sama) ...
        ...
        playSound(failSound, 'GAGAL');
        // ... (Logika UI Gagal tetap sama) ...
...
// ... (Fungsi checkCardSupabase dan HID Listener tetap sama) ...
