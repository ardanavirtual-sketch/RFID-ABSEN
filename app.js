...
// Suara Beep
// =======================
const beepSukses = new Audio("assets/beep-sukses.mp3");
const beepGagal = new Audio("assets/beep-gagal.mp3");


// =======================
// Hubungkan ke RFID Reader
// =======================
async function startRFID() {
  statusDiv.innerText = "Menunggu izin USB...";

  try {
    const port = await navigator.serial.requestPort();
    await port.open({ baudRate: 9600 });

    statusDiv.innerText = "Reader aktif. Silakan tap kartu...";
    ...
    while (true) {
      ...
      const card = decoder.decode(value).trim();
      if (card.length > 0) {
        cekCard(card);
      }
    }
    ...
    statusDiv.innerText = "Gagal menghubungkan ke RFID!";
    console.error(error);
  }
}
...
// =======================
// Cek kartu ke Supabase
// =======================
async function cekCard(card) {

  statusDiv.innerText = "Memeriksa kartu...";
  ...
  statusDiv.innerText = "Tap kartu berikutnya...";
}
